...
export function setupMonacoLanguage(monaco: Monaco) {
  ...
  monaco.languages.setMonarchTokensProvider("mongoShell", {
    keywords: [
      "db", "var", "let", "const", "function", "return", "if", "else", "for", "while",
      "do", "switch", "case", "break", "continue", "new", "this", "true", "false", "null",
      "undefined", "typeof", "instanceof", "in", "of", "try", "catch", "finally", "throw",
    ...
    mongoMethods: [
      ...
      "drop", "rename", "stats",
      "bulkWrite", "watch", "mapReduce",
    ],
    mongoObjects: [
      "ObjectId", "ISODate", "NumberLong", "NumberInt", "NumberDecimal",
      "Timestamp", "BinData", "UUID", "MinKey", "MaxKey", "RegExp", "Date",
    ],
    operators: [
      ...
      "$unwind", "$lookup", "$facet", "$bucket", "$sample",
      "$count", "$merge", "$out", "$replaceRoot", "$addFields",
    ],
    tokenizer: {
      root: [
        // Comments
